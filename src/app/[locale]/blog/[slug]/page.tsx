...
import {
  getAllArticlesAllLocales,
  getArticle,
} from "@/lib/mdxArticles";
...
export async function generateStaticParams(): Promise<Params[]> {
  const articles = await getAllArticlesAllLocales();

  return articles.map(({ locale, slug }) => ({ locale, slug }));
}
...
export async function generateMetadata({
  ...
  const title = `${article.title} | ${t.general.name}`;
  ...
  return {
    ...
    openGraph: {
      ...
      title,
      description: article.excerpt,
      siteName: t.general.name,
      locale: locale === "es" ? "es_ES" : "en_US",
      alternateLocale: locale === "es" ? ["en_US"] : ["es_ES"],
      publishedTime: new Date(article.date).toISOString(),
      authors: [article.collaborator?.name || t.general.name],
      tags: article.tags,
    },
    twitter: {
      card: "summary",
...
export default async function ArticlePage({
  ...
  const jsonLd = {
    ...
    author: {
      "@type": "Person",
      name: article.collaborator?.name || t.general.name,
      url: article.collaborator?.linkedin,
    },
  };
